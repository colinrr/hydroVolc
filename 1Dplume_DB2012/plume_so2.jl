using QuadGK
function plume_so2(pI, pO)
# Function to calculate in-plume SO2 scavenging & stratospheric delivery
# efficiency.
#  --> Need SO2_0 in pI
    ...
    SSAi  = (3.0 / (pI.Rgsd .* pI.rhoi)') .* pO.mi # by grain size [m^2/kg]
    SSA   = sum(SSAi, dims = 2) # Specific surface area of bulk particles [m^2/kg]
...
#     rhop = .002; # kg/m3 particle loading
    ...
    F = so2_eff(pO.hb + pI.vh0, pI.vh0, pO.atmo.ztropo)
#     F = F.*SO2(./pI.SO2_0)
    return [F, m_SO2]
    end


function so2_eff(SO2_height,vent_height,tropo_height)
    ...
    # for lofting induced by radiation absorption & heating
    ...
    SO2prof = (z->begin
        exp(-((z - SO2_height) .^ 2) / (0.108 * (SO2_height - vent_height)) ^ 2)
    end)
    ...
    return quadgk[SO2prof, tropo_height, Inf] / integral[SO2prof, -Inf, Inf]  # stratfrac
    end