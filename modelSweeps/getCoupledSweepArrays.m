function [qA, thresh] = getCoupledSweepArrays(dat,MCflag)
% Retrieves summary data for a set of couple runs. 
...
% Sweeps should use the same atmo profile for all runs for now
ei = 1;
while and(isempty(dat(ei).cI),ei<length(dat))
    ei = ei+1;
end
...
ff = fieldnames(dat); % Order should be: cI cO pI dO wO pO
qA.QCexist = zeros(length(dat),length(ff));
qA.QClevel = zeros(length(dat),3);
for ii=1:length(dat(:))
...
%     [~,ci] = ismember('cI',ff);
    ...
    % 3) MWI fail?
    if qA.QCexist(ii,5) && ~dat(ii).wO.failedPlume
        qA.QClevel(ii,2) = 1;
    end
    
    % 3) Plume fail?
    if qA.QCexist(ii,6) && isfield(dat(ii).pO,'u') && (dat(ii).pO.u(end).*sin(dat(ii).pO.angle(end)))<1
        qA.QClevel(ii,3) = 1;
    end
...
for ii=1:numel(dat)
    ...
    % ---------------------- PLUME OUTPUT ------------------------
    if qA.QCexist(ii,6) && ~isnan(dat(ii).pO.m_0) %isfield(dat(ii).pO,pOf{ff})
        for ff=1:length(pOf)
        ...
        if ~qA.pO.collapse(ii) % Bouyancy level props
            ...
            [~,tpi] = min(abs((dat(ii).pO.z + dat(ii).cI.vh0 + dat(ii).cI.Zw) - qA.htropo));
            ...
            % Tropopause props
            if (dat(ii).pO.z(end) + dat(ii).cI.vh0 + dat(ii).cI.Zw)>=qA.htropo % crossing tropopause?
...
if ~MCflag  % Thresholds mostly relevant for parameter sweeps
    qA.Q0 = qA.cI.Q(:,1);
    % logQ0 = log10(Q0);
    qA.Zw = qA.cI.Zw(end,:);
