...
if isempty(randPars)
    clear randPars
    nRandPars  = length(cfn) + length(pfn);
...
dat(N,1)     = struct('cI',[],'cO',[],'pI',[],'dO',[],'wO',[],'pO',[]);
fn         = fieldnames(dat);
...
fprintf('Running Hydrovolc model Monte Carlo sweep:\n\tN = %i, Conduit: %s, %i Random Params, %i Core(s)\n',N,conType,nRandPars,cores)
...
%         esplode
        tic
        if reRunMC
            [cI,pI] = getMCreRunPars(cI0,pI0,MC,randPars(ii));
        else
            [cI,pI,randPars(ii)] = populateParams(cI0,pI0,MC);
        end
...
%         randPars(ii,:) = randParsi;
    ...
    for ii=1:N
        if reRunMC
            [cI,pI] = getMCreRunPars(cI,pI,MC,randPars(ii));
        else
            [cI,pI,randPars(ii)] = populateParams(cI0,pI0,MC);
        end
...
function [cI,pI,randPars] = populateParams(cI,pI,MC)
...
function [cI,pI] = getMCreRunPars(cI,pI,MC,randPars)
...
function reportOutput(dat,N,ct,rTime,modelFail,randPars)
        ...
        fprintf('  %i/%i:   runtime = %.0f s, Q = %.2e, Ze = %.0f, modFail = %i, Cfail = %i, Wfail = %i, Clps = %i, Z/Ztp = %.2f\n',...
            ct,N,rTime,dat.cI.Q,Zw,modelFail,cF,fP,clp,hm)
