function [cI,cO] = getProxyConduitSource(varargin)
...
%
% CRowell
% - V1.0, Jan 2022. Currently assumes a choked condition at the vent. Will
% return an error if estimated vent pressure is less than ambient.
...
Cm          = 1e-3;       % Residual dissolved water mass fraction
...
Q0      = 1e8;           % Initial magma mass discharge rate (kg/s)
...
%% Calculate params
    ...
    if(ischar(cS.atmo))
        load(cS.atmo)
        if istable(atmprofile)
            cS.atmo = getAtmoArray(atmprofile);
        else
            cS.atmo = atmprofile(~isnan(atmprofile(:,1)),:);
        end
%     elseif(isnumeric(plumeSource.atmo))
%         assert(size(plumeSourc.atmo,2)==5,'Atmo profile must be [m x 5]')
    end
    assert(size(cS.atmo,2)==5,'Atmo profile must be [m x 5]')

    % Get ambient pressure at vent - default from atmo profile
    if isempty(cS.pf)
        cS.pf = 10.^interp1(cS.atmo(:,2),log10(cS.atmo(:,1)),cS.vh0,'pchip','extrap') + rho_e*g*cS.Zw;
    end
...
cO.pg           = P_v;
cO.M3           = 0.7162; % Placeholder for now
cO.Cm           = cS.Cm;
% √ cO.porosity
% √ cO.rho_g        
% √ co.rho_magma
...
function A = getAtmoArray(atmtable)
    ...
    if ismember('Wind_abs',atmtable.Properties.VariableNames)
