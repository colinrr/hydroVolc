...
function [output] = Bubble_Nucleation_Growth_tData(Input)
% By Sahand Hajimirza
...
Par.T           = Input.T + 273.15;                         % Temperature
...
options=odeset('AbsTol',[1e-25,1e-3,1e-12,1e-20,1e-28,1e-2,1e-5],...
               'RelTol',1e-5,...
               'Events',@EventsFcn2);
...
% Preparing output 
    ...
    for i = 1:length(parnames)
        ...
        [par1.(char(parnames(i))); par2.(char(parnames(i))); par3.(char(parnames(i)))];
    end
...
% Added by CR (2021) for additional output params
output.Par    = Par;
...
position = [Par.phi_frag-phi y(6)-1e5 y(7)]; % The value that we want to be zero
...
position = [y(6)-1e5 y(7) .99-M]; % The value that we want to be zero
isterminal = [1 1 1];  % Halt integration 
